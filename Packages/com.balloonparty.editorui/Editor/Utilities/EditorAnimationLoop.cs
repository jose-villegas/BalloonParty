...
namespace BalloonParty.EditorUI.Utilities
{
    ...
    public sealed class EditorAnimationLoop
    {
        ...
        /// <summary>Starts the loop; onTick returns false to auto-stop.</summary>
        public void Start(Func<float, bool> onTick, Action onComplete = null, Action onRepaint = null)
        {
            if (IsPlaying)
            {
                return;
        ...
        public void DrawControls(Action onPlay)
        {
            ...
            if (IsPlaying)
            {
                ...
                EditorGUILayout.HelpBox($"Status: {status}  |  Speed: {TimeScale:F2}×", MessageType.Info);
            }
        }

        /// <summary>Draws a playback speed slider.</summary>
        public void DrawSpeedSlider(string label = "Playback Speed", float min = 0.05f, float max = 3f)
        {
            TimeScale = EditorGUILayout.Slider(label, TimeScale, min, max);
        }

        /// <summary>Test seam: invoke tick logic directly with a given delta.</summary>
        internal void SimulateTick(float delta)
        {
