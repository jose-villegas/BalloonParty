    ...
    public class NaughtyInspector : UnityEditor.Editor
    {
        ...
        protected void DrawSerializedProperties()
        {
            ...
            if (tabGroups.Length > 0)
            {
                ...
                {
                    _selectedTab = new SavedInt($"{PropertyUtility.GetObjectKey(target)}.tab.selected", 0);
                }
            ...
            foreach (var group in GetFoldoutProperties(_serializedProperties))
            {
                ...
                {
                    _foldouts[group.Key] = new SavedBool($"{PropertyUtility.GetObjectKey(target)}.{group.Key}", false);
                }
        ...
        protected void DrawButtons(bool drawHeader = false)
        {
            ...
            if (_groupedMethods.Any())
            {
                ...
                foreach (var group in _groupedMethods)
                {
                    ...
                    {
                        _groupedFoldouts[group.Key] = new SavedBool($"{PropertyUtility.GetObjectKey(target)}.{group.Key}", true);
                    }
