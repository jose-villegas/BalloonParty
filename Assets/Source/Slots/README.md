...
| File | What it does |
|---|---|
| `SlotGrid` | Core data structure — parallel 2D arrays of `IWriteableBalloonModel` and `BalloonView`; `Place`, `Remove`, `At`, `ViewAt`, `IsEmpty`, `IsUnbalanced`, `OptimalNextEmptySlot`, `BottomEmptySlotPerColumn`, `GetNeighbors`, `IndexToWorldPosition`, `OnChanged` |
