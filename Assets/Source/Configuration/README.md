...
| `GameConfigurationEditor` | Custom `Editor` for `GameConfiguration` *(transitional)* — adds the "Show Limits In Scene" toggle that controls `MapLimitsSceneOverlay` (see `Source/Editor/README.md`). Maintained during decomposition; will be deprecated when `GameConfiguration` is fully replaced by its split SOs. |
