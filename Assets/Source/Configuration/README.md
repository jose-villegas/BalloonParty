...
All custom `PropertyDrawer` implementations in this folder extend `AutoFieldPropertyDrawer` (in `Source/Editor/`). Common fields are discovered automatically via serialized-property iteration and rendered using nicified names — drawers only need to declare which fields require special layout in `ExcludedFields` and handle those manually.
