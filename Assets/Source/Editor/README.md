...
| `ProjectileFlightConfigEditor` | `ProjectileFlightConfig` | Adds "Show Limits In Scene" and "Show Aim Fan In Scene" toggles below the default inspector, controlling `MapLimitsSceneOverlay` and `AimFanSceneOverlay` respectively |
...
| `AimFanSceneOverlay` | `[InitializeOnLoad]` static class (in `Configuration/Editor/ProjectileFlightConfigEditor.cs`, alongside `MapLimitsSceneOverlay`) — draws one ray per angle the quantized aim can actually reach, from the thrower's spawn point, clipped to the map limits (`WallLimits.TryFindCrossing`). Angles come from the exact `AimAngleGrid.ResolveSweepSampleCount`/`ResolveSweepAngle` helpers (`Shared/`) the Shot Solver and Fire Best Shot cheat sweep with, over the same config-sourced range (`IProjectileFlightConfig.AimAngleMinDegrees`/`AimAngleMaxDegrees`) the thrower itself clamps to, so the picture can't diverge from what those tools — and the player's own aim — consider reachable. Exists to make visible that a portrait board fired up its long axis does NOT map equal angular steps to equal board spacing — near-vertical rays run long and land far apart near the top, side rays hit the near walls and bunch up; the clipping is what shows this, an unclipped equal-length fan would hide it. Off by default; draws nothing when `AimAngleStepDegrees` is 0 (continuous aim has infinitely many reachable angles, so there is no finite fan to draw). Active in both edit and play mode. Toggle persisted via `EditorPrefs` and controllable from the `ProjectileFlightConfigEditor` inspector |
...
| `ReleaseUploadWindow` | `Tools > BalloonParty > Upload Release` | Builds all three APK variants (release, release+cheats, development), checksums them, then publishes a tagged GitHub release with the APKs attached. `ReleasePublisher` does the API half in C# — no `bash`, `curl`, `jq` or `gh` needed, so it runs the same on Windows as on macOS. Paste the token into the window; it is never written to disk, a log, or a process command line |
