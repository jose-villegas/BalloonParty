...
{
    /// <summary>
    ///     Optional capsule (segment) lights mirroring the prediction line while the player aims — one per
    ///     trace leg (the calculator emits corner points only: launch, each wall bounce, the top-wall
    ///     tip), so the lights bend exactly where the line does. Gated entirely by <see
    ///     cref="IPredictionTraceConfig.LightingEnabled" />; every call is a no-op when it's off, so a
    ///     caller can wire this in unconditionally. Intensity and half-width are tunable from launch to
    ///     tip via <see cref="IPredictionTraceConfig.LightFadeCurve" />/<see
    ///     cref="IPredictionTraceConfig.LightWidthCurve" />, sampled at each leg's arc-length position.
    ///     Lights register when the trace appears, are mutated in place on every aim update (re-registering
    ///     per frame would churn the field), and are disposed when the trace clears.
    /// </summary>
    internal sealed class PredictionTraceLights : IDisposable
    {
        ...
        internal PredictionTraceLights(SceneLightFieldService lightField, IPredictionTraceConfig config, int paletteIndex)
        {
        ...
        internal void SetTrace(IReadOnlyList<Vector3> points)
        {
            ...
            var baseIntensity = _config.LightIntensity;
            var fadeCurve = _config.LightFadeCurve;
            var halfWidth = _config.LightHalfWidth;
            var widthCurve = _config.LightWidthCurve;
        ...
        private void Rebuild(int legCount)
        {
            ...
            for (var i = 0; i < legCount; i++)
            {
                var light = Light.Segment(Vector3.zero, Vector3.zero,
                    _config.LightHalfWidth,
                    _config.LightIntensity,
                    _paletteIndex,
                    _config.LightFalloffPower);
                _lights.Add(light);
