...
| `ToughBalloonVariant` | `MonoBehaviour` implementing both `IBalloonVariant` and `IBalloonViewBinding` — not colorable; `Initialize()` is a no-op. On `Bind()` subscribes to `HitsRemaining` (via `IHasDurability`) and animates `_DamageProgress` on the `BalloonParty/Balloon/ToughBalloon` shader via `MaterialPropertyBlock` using a DOTween tween. Injects `SmokeFieldService` and, when down to one hit, paints into the smoke trail at the repel apex of its breathing cycle (`PaintSource.ToughBreathing`) |
