...
- **BalloonBalancer** — moves balloons when gaps appear; runs once before spawning (pre-spawn consolidation) and once after (deferred via `BalanceBalloonsMessage`). Reserves transit slots in `BalancePathHolder` per-actor; releases on animation complete
