...
- **ScoreController** — on each `ActorHitMessage` with `Outcome == Pop` or `PassThrough`, casts actor to `IHasScoreColor` and calls `ResolveScoreAttribution(msg.Context, attributions)`. Each returned `ScoreAttribution` drives one or more `ScorePointMessage` publications (one per point × streak). A legacy fallback for `IHasColor and IHasScore` remains until all actors implement `IHasScoreColor` (removed in Phase 2)
