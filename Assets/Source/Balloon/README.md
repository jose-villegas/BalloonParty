...
| `Controller/` | `BalloonController` — wires model to view and handles hit/deflect/pop routing via `msg.Outcome` switch; `BalloonBalancer` — rebalances the grid after each turn; reserves transit slots in `BalancePathHolder` per-actor, released on animation complete |
