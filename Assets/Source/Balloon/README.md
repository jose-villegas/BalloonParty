...
| Folder | What it owns |
|---|---|
| `Model/` | `IBalloonModel` (read-only interface — extends `IDynamicSlotActor`, `IHitable`, `IHasScore`, `IHasNudge`), `IWriteableBalloonModel` (mutable — extends `IWriteableDynamicSlotActor`), `BalloonModelBase` (abstract — implements `IWriteableBalloonModel, IHasDurability`; constructed via `BalloonModelConfig`; `EvaluateHit` supports `DamageFlags.Piercing` for forced `Pop`), `BalloonModel` (concrete — extends `BalloonModelBase`, also implements `IHasWriteableColor` and `IHasWriteableItemSlot` — both paintable and item-capable), `ToughBalloonModel` (concrete — extends `BalloonModelBase`, NOT paintable, NOT item-capable; `EvaluateHit` returns `Deflect` on survival). `IHasWriteableItemSlot` (in `Model/`) extends `IHasItemSlot` (in `Slots/Capabilities/`) with a writable `ReactiveProperty<ItemType> Item`. Item eligibility is determined by type — `BalloonModel` implements the interface; `ToughBalloonModel` does not. Consumers use the narrowest interface: views receive `IBalloonModel`; controllers, spawners, and grid operations use `IWriteableBalloonModel`. `IHasColor` is NOT on `IBalloonModel` — only `BalloonModel` implements it; consumers that need color cast to `IHasColor` |
