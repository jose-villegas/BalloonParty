...
4. **Reject** — only when nothing frees a slot: `RejectedBalloonEffect.Play` queues the would-be balloon into the overflow pile below the grid; it lingers, then a heart trail is sent to it and it pops (costing a hit point) when the heart lands.
