...
| `Snipe/SnipeItemHandler` | Snipe — applies a `SpeedProjectileBuff` via `IProjectileBuffs.Apply` (see `Projectile/Buffs/`), boosting the active projectile's flight speed (multiplier from `SnipeSettings.SpeedBuffMultiplier`) until it loses a shield to a wall. Pure buff grant: no damage, no shield change, no rainbow-specific behaviour |
...
| **Snipe** | Snipe icon, tinted to host color | Boosts the active projectile's flight speed (configurable multiplier via `SnipeSettings`) until it next loses a shield to a wall (applies `SpeedProjectileBuff`) | N/A — no damage dealt |
