...
"Pen-up" is wider than just the ribbon: `HighlightTrail.SetEmitting` toggles the head sprite alongside
the `TrailRenderer`, so a parked or approaching pen shows neither a ribbon nor a lone dot sliding
around with no ribbon behind it — the head is part of "is this pen drawing," not decoration independent
of it. Every place the ticker starts or stops a pen drawing (`AcquirePens` parking a fresh pen, the
...
visibly emerges from the balloon — and it travels pen-up the whole way, exactly as before, for the same
reason the old shared leg had to stay dark: it is the only pen that ever takes that path, so there is
nothing left to stack. See the `ItemPreviewTicker` row above for the full mechanics (`AdvanceCascade`,
`BuildCascadeTimings`) and how the standing rule below is satisfied throughout.
