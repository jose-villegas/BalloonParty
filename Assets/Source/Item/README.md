...
**The figure loops: draw in, hold, fade out, then replay.** Once a figure's draw-in finishes,
`ItemPreviewTicker` doesn't leave it settled forever — it walks an explicit three-phase state machine
(`CyclePhase`: `Drawing` → `Holding` → `Fading`) off a single ticker-level clock (`_cycleElapsed`,
advanced in `LateTick` via `AdvanceRebloomCycle`) rather than polling every pen for "am I settled yet,"
since every pen already ticks against one shared `BloomElapsed`-derived clock by construction. `Drawing`
lasts `BloomDuration`, `Holding` lasts `IItemPreviewConfig.RebloomHoldSeconds`; once `Holding` elapses,
`BeginLoopFade` stops every pen emitting and lets their ribbons age out on their own lifetime — the
figure visibly fades before the replay starts, rather than vanishing in one frame — and only once that
fade elapses does `RestartCascade` replay the whole cascade from scratch and the phase return to
`Drawing`, for as long as the preview stays up. `RebloomHoldSeconds == 0` is the off switch: never
advance the phase at all, draw once and hold, exactly the behaviour before this existed, so the loop is
opt-in rather than a default nobody can turn off.
...
apart: only `BeginLoopFade` ever leaves it at `Fading`, so `AdvanceFade` replays the cascade in that case
and releases the pens to the pool in every other case (a genuine `BeginHide`). A `BeginHide` arriving
while the loop's own fade is already running still wins outright — it redirects that same in-flight fade
from a replay to a release rather than starting a second one on top of pens already stopped and already
ageing, so the preview still disappears for real rather than blooming back in.
...
the figure would leave the comet's tail dead forever after the first cycle. `RestartCascade` reuses the
exact per-pen reset `AcquirePens`/`AcquireApproachPens` already use to introduce a pen the first time
(`ResetPenForDrawIn` / `ResetApproachPenForDrawIn`, factored out of both so "what does it mean for this
pen to begin cascading in" has exactly one definition instead of a second copy that could drift from the
original) — every pen goes back to `Parked`/pen-up at the host origin with its ribbon cleared and not
emitting, exactly the state `AdvancePen`'s rising edge is built to draw cleanly out of. This is
deliberate, not incidental: a settled pen is mid-ping-pong somewhere along its own dash, so a restart
that jumped it straight back to its dash start while still emitting would draw a straight chord across
the whole slot — the exact discontinuous-emit failure the standing rule above exists to prevent, and one
this system has produced four different ways already; going through the acquire path's own reset, rather
than writing a fresh one inline, is what keeps a rebloom restart from being a fifth. The fade only makes
this safer still: every pen has already stopped emitting and sits frozen before `RestartCascade` ever
repositions it, so there is no window where a pen could be emitting at a position it's about to leave.
What a restart does **not** touch: the shape, strokes, arc tables, entry offsets, dash counts, or
cascade timings — none of that changes between cycles, so `RestartCascade` never re-derives geometry,
never touches the pool (no `Get`/`Return` churn per cycle), and allocates nothing; only the pens' own
cascade state resets. `Show` resets the loop's phase and clock unconditionally, so a fresh `Show` (a new
host, or a fade-in resuming the same one) never starts mid-hold or mid-fade, and can never race an
already-due rebloom on the same frame; the clock only advances while the figure is visible and not
fading, since `AdvanceRebloomCycle` runs behind the same `LateTick` gating that already guards advancing
the pens themselves — a hidden or fading figure never queues up a rebloom that fires the instant it
reappears.
