...
(`_shown == true`) a `true` reading calls `AdvanceSequence`: step `_sequenceIndex` to the next entry in
`_sightedHosts`, wrapping past the last back to the first, and `Show` that host's figure — always
`introduce: true`, rather than `ShowActiveHost`'s own introduce rule (which reads "same set as last
shown" as a re-settle, not a fresh draw-in — exactly backwards for a cycle that just parked). A
single-host line is a one-element sequence, so it advances to itself (`0 -> 0 mod 1`) every cycle — the
sequence collapses to exactly today's single-host loop rather than needing a special case for it. This
...
signature (`HasActiveSignatureChanged` never reads it), and a settled angle change is instead its own
re-bloom request: `ItemRangePreviewController.HasSpinSettledOnNewAngle` compares the raw angle (all the
controller ever sees, via `IHostsSpinningItem`/`ISpinningItemVisual`) against last frame's reading to
tell a lerp still in flight (changing every frame) from one that just arrived and is now dwelling
(identical frame to frame, but different from the angle the figure was last drawn at) — no
transition-phase signal from `LaserItemRotation` itself is needed. A settled change calls `Show` again
with `introduce: true` on the *same* host, redrawing the figure from scratch at the new orientation
without touching `_shown`, the dwell timer, or the sequence position — the loop stays running, the
sequence doesn't move.
<br><br>
Because `Show` always resets `ItemPreviewTicker`'s own draw/hold/fade clock, every one of those
re-blooms restarts the cycle a spin-driven host is otherwise measured against, so a Laser rotating
faster than one full cycle would still never let `CycleComplete` fire on its own — the same problem one
level up. `_activeHostElapsed`, a per-host accumulator that ticks every frame a sequence entry stays
active and resets only when the sequence actually advances or the sighted set changes — **never** by a
re-bloom — is what bounds its turn regardless: once it reaches one cycle's worth of time
(`BloomDuration + RebloomHoldSeconds`, the same terms `ItemPreviewTicker.AdvanceRebloomCycle` already
uses for its own draw + hold, rather than a new authored knob), the next settled angle advances the
sequence (`AdvanceSequence`) instead of re-blooming the same host again. This only ever fires at a
settled-angle boundary — a dwelling moment, not mid-lerp — so it never interrupts a draw-in the way an
advance triggered on an arbitrary frame could. Gated on `RebloomHoldSeconds > 0`, mirroring
`AdvanceRebloomCycle`'s own off switch: with looping authored off, `CycleComplete` never fires for
*any* host, spinning or not, and this must not either — a Laser with looping off just re-blooms in place
on every rotation update, forever, exactly as a non-spinning host's first draw simply holds forever.
Non-spinning items are unaffected end to end: their spin reads a constant `0`, so
`HasSpinSettledOnNewAngle` never reports a change and they keep advancing purely on `CycleComplete`,
exactly as before.
