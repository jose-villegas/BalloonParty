...
| `Lightning/ChainLightningView` | `EffectView` subclass for chain lightning — multiple `LineRenderer`s; `PrepareDisplay(positions, segMul, randomness, jumpTime, onTargetHit)` stores target data; `Play()` starts `async UniTaskVoid` that grows forward jump-by-jump then retracts. `BuildBoltBuffers` and `FillSegment` are `internal static` — shared with editor preview (single source of truth for bolt math) |
