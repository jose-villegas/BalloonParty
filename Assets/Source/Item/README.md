...
`AdvanceSequence` updates only the signature's **active-host** geometry (origin, direction, spin, trace
end) to the newly-active host's own values, never `_signatureSlots` — the sighted set hasn't changed,
only the sequence's position within it, and routing the advance through the same full reset `StoreSignature`
uses for a genuine set change would make the very next frame's comparison read the advance itself as a
changed aim (`HasActiveSignatureChanged` would compare the new host's geometry against the old one's
stored snapshot and see a "jump"). This is the reason the signature is split into two parts at all: a
**set** half (`_signatureSlots`, exact per-slot equality, changes only when the aim genuinely moves onto
a different group of balloons) and an **active-host** half (the continuous origin/direction/spin/trace-end
of whichever host is currently drawing, changes on every advance *and* whenever that host itself drifts).
...
"where it left off" to resume from.
