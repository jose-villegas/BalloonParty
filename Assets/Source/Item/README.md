...
| `Snipe/SnipeItemHandler` | Snipe — arms the active projectile as a piercing lance: sets `IsPiercing` so it plows through balloons like a cruise-earned pierce, but **without** entering cruise (so it never gains cruise's per-shield speed tap). Grants a single, **non-stacking** multiplicative `ProjectileBuffId.Speed` buff (multiplier from `SnipeSettings.SpeedBuffMultiplier`) via `IProjectileBuffs.Apply` (see `Projectile/Buffs/`) — a second Snipe only re-arms the (idempotent) pierce, it does not compound the speed. Plowing a tough/unbreakable balloon (`hits > 1`) bleeds the speed back down toward true base via the pierce-speed decay in `ProjectileMotionResolver` (now keyed off `IsPiercing`); the next wall after a tough plow spends the pierce and ends the speed buff, its lifetime tied to the pierce by `PierceEndedEndCondition`. With no tough contact it pierces until it runs out of shields. Non-damaging: no shield change. (Terminal line-clear and the rainbow charge/bloom are later phases — not yet shipped) |
...
| **Snipe** | Snipe icon, tinted to host color | Arms the active projectile as a piercing lance — it plows through balloons (like a cruise-earned pierce, but without cruise's per-shield speed tap) and gains a single non-stacking speed boost (multiplier via `SnipeSettings`); plowing a tough balloon bleeds the speed back toward base, and the next wall after a tough plow spends the pierce and ends the boost | N/A — no damage dealt |
