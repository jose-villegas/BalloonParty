...
| `ItemPreviewEntry` | Pure polyline-vs-polyline math — where the aim's own trace crosses one stroke, as an arc-length offset along that stroke — factored out so it is edit-mode testable without the ticker's pen/pool machinery, mirroring `TraceHitGeometry`. `TryFindEntryOffset` walks the trace segment by segment (not the stroke segment by segment) so "along travel" means ordered by the shot's own direction of travel, including a closed stroke's wrap leg as a segment; ties within one trace segment are broken by the intersection nearest that segment's own start. It prefers the **second** crossing along travel — a figure's cascade starts at the sight's **exit** from it, not its entry, so the approach leg is now expected to run through the figure's near side on its way to that exit — falling back to the only crossing when the sight crosses the stroke just once (Shield's stub, Laser's lines meeting at the host, a Lightning arc reached only through the chain, Snipe's corridor anchored near-collinear with the trace itself, where a second crossing genuinely isn't there to find). Two crossings resolved within a tiny arc-length epsilon of each other collapse to one rather than counting as a real second — otherwise a graze past a shared vertex, or a near-tangent line, would register as two crossings a hair apart and the pick would flicker between them as sub-pixel aim jitter decided which one floating-point rounding found. Alongside the stroke-side offset it also reports the crossing's own **trace-side** arc offset — the distance from the trace's first point to the crossing, measured along the (possibly bent) trace itself — which is what lets the stroke's approach cascade follow the trace instead of cutting a straight line to it (see `ItemPreviewTicker` below). Returns `false` (both offsets `0`) when the trace never crosses the stroke at all — `ItemPreviewTicker` treats that as "start this stroke's draw-in at its own beginning, and fall back to a straight-line approach" rather than a failure. `TryFindNearestPointOnPolyline` is the other shared projection: the closest point on an arbitrary polyline to a target, plus that point's own arc-length offset from the polyline's first point. `ItemPreviewTicker` calls it once per `Show`/refit to place the host on the retained trace copy (`_hostTraceOffset`, anchoring the approach cascade); `SnipeRangePreview` calls it directly to anchor its pierce corridor on the same trace instead of the host centre. False when the polyline has fewer than two points, the same degenerate case its caller falls back from. `AppendApproachLoop` is the third pure helper here: appends a closed loop around a centre at a given radius to a point buffer, its starting vertex sighted toward wherever the caller's own leg already starts (falling back to a caller-supplied direction, then a fixed axis, when that leg starts on the centre itself) so the loop and whatever polyline follows it connect via the shortest bridge rather than an arbitrary one. `TryFindCircleCrossing` is the fourth: where a trace polyline crosses a circle, walked outward segment by segment from a given arc-length offset on whichever side the caller asks for (a real line/circle intersection per segment, so it neither assumes the crossing sits exactly one radius along the trace from that offset — the true half-chord is `sqrt(r² − d²)` for a perpendicular offset `d`, not `r` — nor that it lands on the same segment the starting offset does) — false when the trace has fewer than two points, the radius is non-positive, or the walk runs out before ever reaching the circle's edge on the requested side. See `ItemPreviewTicker`'s approach-loop paragraph below for how both are used together to anchor the loop on the trace's own crossing of the balloon |
| `ItemPreviewContext` | Readonly struct — the per-crossing inputs (`Origin`, `Slot`, `AimDirection`, `TracePoints`, `HostColorId`, `ItemSpinDegrees`, `TraceEnd`, `Viewport`, `HostBalloonType`). The services a preview needs are constructor-injected into the preview itself, since they never change between crossings. `HostBalloonType` exists solely so `ItemPreviewTicker` can key `BalloonContactRadii` by the host's own type for the approach loop's radius, without the ticker reaching into `SlotGrid` itself |
