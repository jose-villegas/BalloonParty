...
| `IItemPreviewStyle` | One item's override, opt-in: `SkipBloom`, whether this item's figure appears in place instead of playing the outward bloom (default `false`, so an unauthored entry blooms like everything else). Used to be a tri-state `BloomDraw` deferring to a shared "draw during bloom" flag, but that only ever affected an *in-progress* bloom — and once the bloom became an introduction-only event (see `ItemPreviewTicker` below), a non-introducing show starts already `Bloomed = true` with nothing for it to affect. A plain "does this figure bloom at all" is the knob that's actually reached for now, so it needs no shared counterpart to defer to. Deliberately carries **no colour**: every figure draws with the pen prefab's own material, so the telegraph reads as one system and there is no runtime tint path to keep in step with it |
...
a pen emitting at the origin used to draw a straight chord out to wherever it first computed as its
real position, since a pen that starts `Bloomed = true` — every `SkipBloom` item, and every pen on a
same-host re-settle, which is the common case — jumps straight to its settled place in the figure on
that very first tick. This is the third distinct way this system has drawn an unwanted straight line
(alongside the re-entry and reassigned-slot cases above), which makes it worth stating as a standing
rule rather than three separate fixes: **a pen must never be emitting at a position it is about to
leave discontinuously.**
...
centre into its place in the figure plays by default, which suits a figure centred on the host
...
`SkipBloom` to `true` so its figure just appears in place instead.
