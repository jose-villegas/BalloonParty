...
`ClampAndQuantizeAimDirection` sidesteps both failure modes by delegating to `ShotBoardGather.ClampToReachableAngle`, which clamps the *rounded grid index* rather than the angle — the result is always both a step multiple and inside the range, by construction. This is the same absolute grid `ShotBoardGather.ResolveSweepAngle` sweeps with (see `Solver/README.md`), so the thrower's own clamp and the solver's sweep — and the Scene-view aim fan overlay, see `Editor/README.md` — can never disagree about which angles are reachable: one config-sourced range, consumed everywhere by construction rather than by convention.
