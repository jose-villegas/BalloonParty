...
Every tap **before** the arming one keeps its beat, unchanged — that dip is how the build-up reads. The **arming tap** is the one exception: instead of replaying the beat it starts the *arm ramp* (`PierceArmRampDuration` / `PierceArmRampCurve`). Without it the shot holds full speed from the arming bounce on and reads as snapping to top speed rather than earning it. On the solver's event timeline the same ramp is a pure time cost, scaled by the speed gap it still has to close (`ShotCruiseConfig.ArmRampLagSeconds`).
...
Velocity-scaled feedback normalizes against `ProjectileMotionResolver.ReachableTopSpeedMultiplier` — the arming tap's multiplier — **not** `MaxCruiseSpeedMultiplier`. Since taps freeze at arming, the configured cap is unreachable while piercing is enabled, and normalizing against it pins every such effect in the bottom fraction of its curve.
