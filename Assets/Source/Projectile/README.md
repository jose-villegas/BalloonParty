...
**Taps stop once the shot is armed.** A piercing shot already sits at the top speed its taps earned, so neither a cruise bounce nor a sweep pays out another one (`ProjectileMotionResolver.Step` and `ProjectileView.TryAwardSweepTap` both gate on `IsPiercing`) — its ramp is frozen for the rest of the pierce, and the per-tap freeze-then-pickup envelope stops replaying with it (with no speed change left to sell, that dip reads as a hitch and would keep ducking the pierce aura). This is also what stops a Snipe lance — which deliberately never enters cruise — from picking up the cruise ramp sideways through sweeps.

Every tap **before** the arming one keeps its beat, unchanged — that dip is how the build-up reads. The **arming tap** is the one exception: instead of replaying the beat it starts the *arm ramp*, a single acceleration from the speed the shot was actually travelling at into its now-frozen top speed (`PierceArmRampDuration` / `PierceArmRampCurve`, anchored by `ProjectileModelExtensions.ArmPierce`). Without it the shot holds full speed from the arming bounce on and reads as snapping to top speed rather than earning it. The ramp blends between two real speeds rather than scaling a target, so an unauthored or 0-start curve holds the arming speed instead of stalling the shot, and `0` duration disables it. On the solver's event timeline the same ramp is a pure time cost, scaled by the speed gap it still has to close (`ShotCruiseConfig.ArmRampLagSeconds`).
