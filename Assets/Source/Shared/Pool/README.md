...
- **`PoolChannel<TItem>`** — abstract base owning a `Stack<TItem>`. `Get()` pops or calls `Create()`; skips destroyed items in the stack. `Return()` despawns, deactivates, and pushes back. On return, `SetParent(..., worldPositionStays: false)` prevents scale drift from reparenting — skipped if the item is already parented under `Container`, so channels whose items never leave their home transform (see `Register(key, channel, container)` below) pay no per-return `SetParent` cost. Subclasses implement `Create()`.
- **`PoolManager`** — injectable singleton registry keyed by `string`. Channels are registered via `Register()` (creates a container `GameObject` under the shared `[Pool]` root) or `Register(key, channel, container)` (homes the channel under a caller-supplied `container` instead — for pools whose items should live where they're consumed, e.g. a HUD notice pool parented under its own bar), or lazily via `GetOrRegister()`. Consumers call `Get<TItem>(key)` / `Return(key, item)`. `KeyFor(prefab)` returns a cached pool key for a prefab — `Object.name` allocates a fresh managed string on every access, so hot paths (per-pop VFX) resolve keys through the cache instead.
