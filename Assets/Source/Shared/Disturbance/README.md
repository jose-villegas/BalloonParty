...
- **`Sprite/DisturbanceTint` shader** — a sprite tinted by the field at its world position: the A-channel tag colours it (`_DisturbancePalette[index]`, strength = tag life), and the G/B direction can rotate the sprite to face the flow (base-angle offset, eased by field magnitude) and/or brighten it along that direction. Reads the same globals; no per-instance wiring. Uses vertex texture fetch for the per-sprite direction (Vulkan/Metal)
