...
| `Pause/` | Broadcast pause coordination and time-scale ownership — `PauseService` (reference-counted per `PauseSource`; live systems gate on its `IsAnyPaused` reactive property), `TimeScaleService` (sole legal writer of `Time.timeScale`; claim/release by `TimeScaleSource`, lowest active claim wins; exclusive claims override the min-wins stack for authored ramps), `ITimeScaleClaims` (write surface interface — controllers depend on this for testability) (see `Pause/README.md`) |
...
| `SceneLight/` | Shared light field — the disturbance-field architecture applied to light: `TimeOfDayService` (owns + pushes the ambient globals `_SceneLightDir`/`_SceneLightColor`/`_SceneLightIntensity`, exposed live via `ISceneLightRuntime`), `SceneLightFieldService` (owns the local field RT), `Light` (reactive per-light model; `RegisterLight`/dispose turns it on/off). Consumers sample it through `SceneLight.cginc`. See `SceneLight/README.md` |
...
| `AimAngleGrid` | The thrower's reachable-angle domain rule — the absolute step grid a quantized aim snaps to (`ClampToReachableAngle`) plus the paired sweep-sample grid (`ResolveSweepSampleCount`/`ResolveSweepAngle`). One domain rule, four independent consumers: the live `ThrowerController`, `FireBestShotCheat`, the editor `ShotSolverWindow`, and the Scene-view `AimFanSceneOverlay`. Used to live on `Solver/ShotBoardGather`, which meant live gameplay (`ThrowerController`) depended on an analysis/planning subsystem to aim — backwards, since the shared thing is a game rule about which angles the thrower can reach, not solver logic; moved here for exactly the reason `WallLimits` is here |
...
| `ScoreLevelUpMessage` | `LevelController` | `ColorProgressBar`, `LevelUpPopUp`, `ColorStreakTracker` (auto-resets the streak), `ScoreController`, `ThrowerController`, `BalloonSpawner`, `PlayerHealthController`, `LevelDifficultyResolver`, `WallNetView`, `ProgressionSoundRouter` |
...
| `ProjectileDestroyedMessage` | `ProjectileView` | `ThrowerController`, `BalloonSpawner`, `BalloonBalancer`, `ProjectileFacingSource`, `ShieldCounterAnimation`, `LevelController`, `CombatSoundRouter` |
