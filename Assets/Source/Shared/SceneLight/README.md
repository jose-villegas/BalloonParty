...
| `ISceneLightRuntime` | Read-only interface for the LIVE ambient light (`CurrentDirection`/`CurrentColor`/`CurrentIntensity`), implemented by `TimeOfDayService`. Inject this instead of `ISceneLightSettings` when a system needs the runtime value rather than the static rest config |
