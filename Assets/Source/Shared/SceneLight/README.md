...
|---|---|
| `TimeOfDayService` | Plain-C# DI service (`IStartable`/`ITickable`, implements `ISceneLightRuntime`). Registered differently per scope: the launcher (`LaunchLifetimeScope`) has no other entry point, so it uses `RegisterEntryPoint` to set up the `IStartable`/`ITickable` dispatcher itself; the game scope (`GameScopeRegistration`) uses a plain `Register(...).AsImplementedInterfaces().AsSelf()`, relying on another entry point to have already wired the dispatcher (same pattern as `DisturbanceFieldService`). Single writer of the ambient globals (`_SceneLightDir`/`_SceneLightColor`/`_SceneLightIntensity`), seeded from `ISceneLightSettings` and pushed once at startup (re-pushed every tick only in the editor, for live tuning). Exposes the live direction/colour/intensity through `ISceneLightRuntime` for CPU-side consumers (sky tint, GI) that can't read shader globals. Holds the current toward-light direction but doesn't decide it — at rest it's the authored value, and while night mode is on it's driven by whichever source `ITimeOfDaySettings.Source` selects: `TimeOfDayCycle` (`Game/Level`, the level-paced sweep) or `TimeOfDayClock` (below, the wall-clock). See @ref plan_night_mode |
| `TimeOfDayClock` | Plain-C# entry point — the `Realtime` night-mode source: rotates the direction continuously on unscaled time, one full circle per `SecondsPerCycle` (30 min default), starting from the authored rest direction. Lives here (no level dependency), so it's registered in both the game and launcher scopes and cycles the menu too. Inert unless night mode is on AND `Source == Realtime`; the level-paced alternative (`TimeOfDayCycle`) guards on `Source == LevelSweep`, so exactly one drives. Per-scope (resets to the rest direction on each scene load, not one persistent clock) |
