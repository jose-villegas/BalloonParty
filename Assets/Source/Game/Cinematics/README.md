...
`LevelUpCinematic` produces **one** cinematic per level-up (the runner's form: `TryBegin` … `EndPanIn`). When `Phase` enters `Completing`, the camera zooms in and follows the **projectile** (via `ProjectilePositionProvider`) until the first wall hit ends the level-complete hit segment. `EndPanIn` then clears the cinematic state while leaving the camera zoomed for the ceremony hand-off. On dismiss, `LevelUpCinematic` only cleans up its session state — the camera un-zoom is driven by the Ascent (`Game/Level/LevelTransitionController.cs`) via `CinematicCameraRig.RestoreTweened`, timed by the `LevelCompleteRestore` segment's own `TimeScaleCurve` duration — independent of the (concurrent) board-clear effect — not by this producer. `LevelController` owns the navigation return to `Game` once the ceremony resolves. The Ascent moves the incoming *scenario*, not the camera (see `LevelAscendCinematic.cs` below).
...
| **End trigger** | First `WallHitMessage` (Window A boundary). Safety cap (5s) as fallback |
| **End** | `EndPanIn()` → ends the hit state while the camera stays zoomed for the popup/Ascent hand-off. Popup gate can now open because `Cinematic.Current != LevelCompleteHit` |
...
| File | Role |
|---|---|
| `LevelUpCinematic.cs` | Level-up producer (plain C# `IStartable`): when `Phase == Completing` and a score point lands, begins the hit beat immediately (no trail wait). Camera follows the **projectile** (via `ProjectilePositionProvider`) until the first wall hit ends the level-complete hit segment (Window A boundary). On dismiss it only cleans up its own session — the camera un-zoom is the Ascent's job (`LevelTransitionController` calls `CinematicCameraRig.RestoreTweened`, timed by the `LevelCompleteRestore` segment's own curve), and `LevelController` owns the nav return to `Game`. **The loss wins**: aborts mid-hit if `ILossForecast.LossImminent` becomes true, publishing `LevelUpAbortedMessage` so the pending ceremony can recover cleanly |
