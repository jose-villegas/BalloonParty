...
| `LevelAscendCinematic.cs` | Level-transition producer (plain C#, not `IStartable` — constructed and driven directly by `Game/Level/LevelTransitionController.cs`): a literal vertical camera translate up and back down (not a zoom), driven by its own per-frame `UniTask` loop off `CinematicCameraRig.PrepareAscend()`/`TranslateAscend(offsetY)`, bypassing `CameraRigCinematic` entirely (this is a plain curve-sampled position, not a pan-toward-focus + tweened restore). Fires a caller-supplied callback once, partway through the descent, so the new level's balloons are already mid-spawn by the time the camera settles. Repurposes `CameraRigCinematicSettings`' generic fields for this one state (documented on the `CinematicsSettings.cs` entry): curve VALUE = height fraction (not timeScale), `ZoomAmount` = ascend height in world units, `PanWeight` = balloon-spawn-cue fraction of total duration, `FollowSpeed` unused |
...
| `CinematicCameraRig.cs` | The **one shared** cinematic camera driver (DI singleton): `PreparePanIn(segment)` / `Frame(focus, segment, dt)` / `PrepareRestore` / `Restore` / `PrepareAscend()` / `TranslateAscend(offsetY)`. Tuning comes per call from the active state's segment; the focus supplies what to frame |
