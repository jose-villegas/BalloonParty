...
`LevelUpCinematic` produces **one** cinematic per level-up (the runner's split-phase form: `TryBegin` … `EndPanIn` … gate). The pan-in zooms the camera onto the tipping trail; `EndPanIn` ends the cinematic but leaves the camera zoomed. On dismiss, `LevelUpCinematic` tweens the camera back to base itself (`CinematicCameraRig.RestoreTweened`, over the Ascent's duration) — the Ascent (`Game/Level/LevelTransitionController.cs`) moves the incoming *scenario*, not the camera, so it can't do that for us (see `LevelAscendCinematic.cs` below).
...
| **`LevelUpCinematic.OnDismissed`** | Tweens the camera back to base (`RestoreTweened`, re-enabling the ortho controller), `Resume(Cinematic)`, finalizes its own session (`NavigationState.Game`). `CinematicState.LevelUpRestore` is no longer played (kept in the enum/settings for serialized-index stability); the un-zoom is a plain rig tween, not that restore-segment machinery |
