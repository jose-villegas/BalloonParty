...
## Level-Complete Projectile Follow

`LevelUpCinematic` produces **one** cinematic per level-up (the runner's form: `TryBegin` … `EndPanIn`). When `Phase` enters `Completing`, the camera zooms in and follows the **projectile** (via `ProjectilePositionProvider`, box-in-box clamped so the frustum never leaves the play area) for the authored `LevelCompleteHit` curve duration. The end trigger is **duration-based** — when the curve's elapsed time reaches its length, `EndPanIn` clears the cinematic state, starts a **curve-driven camera restore** (`Rig.RestoreCurveDriven` using the `LevelCompleteRestore` segment's curve, still following the projectile box-in-box clamped), and leaves the camera un-zooming back to base while the projectile finishes its flight. On dismiss, `LevelUpCinematic` only cleans up its session state — `LevelController` owns the navigation return to `Game` once the ceremony resolves. The Ascent moves the incoming *scenario*, not the camera (see `LevelAscendCinematic.cs` below).

### Cinematic 1 — Hit (`CinematicState.LevelCompleteHit`)

| Step | What happens |
|---|---|
| **Trigger** | `ScorePointsGroupMessage` received while `Phase == Completing` (the tipping claim has already fired) |
| **Begin** | `Runner.TryBegin()` starts the hit beat immediately — no trail wait, no pause. Camera begins following `ProjectilePositionProvider` |
| **Tick** | Camera pans toward projectile position, clamped so it stays within the orthographic frustum. Loss-imminent check aborts if the loss becomes certain |
...
| **`LevelUpCinematic.OnDismissed`** | Finalizes its own session state. It does **not** touch navigation or the camera (the camera restore was already started at `EndPanIn` via `RestoreCurveDriven`; the nav return to `Game` is `LevelController`'s job after the transition completes). `CinematicState.LevelCompleteRestore` is no longer played as a live cinematic state (kept in the enum/settings for serialized-index stability) — its curve is only read by the `RestoreCurveDriven` call |
| **`LevelTransitionController`** | Driven by `ILevelProgress.Phase → Transitioning` (the dismissal advances the phase; the Ascent watches the phase, not the message). Waits for `!Cinematic.IsPlaying` (safety net) then for overflow drain, and runs the Ascent. The camera is already restoring (driven by `LevelUpCinematic.EndPanIn` via `RestoreCurveDriven`); `LevelTransitionController` does **not** touch the camera. Runs an injected `IBoardEffect` to clear the old balloons — bound to `BoardFloatAwayEffect`, so level-up balloons float away rather than pop. Both halves of the old level travel with the descent: statics ride via `ITransitionOutgoingContent`, and the old balloons are detached off the grid + reparented under the `ScenarioContentRoot` (`BalloonControllerRegistry.DetachOutgoing`) by the float effect's `Collect()` — hence the root-reset happens *before* `Collect()` so their reparent lands at the right offset. The level-up's **frozen score trails** are outgoing content too: `ScoreTrailService.HoldOutgoing` resolves them here (`CompleteAll`) so they disappear under the Ascent (see *Pausing survivors through the ceremony*) |

### Pause Integration

| System | Mechanism |
|---|---|
| Projectile | **Not paused** during the level-up — the shot keeps flying through `Completing`. The exclusive `TimeScaleSource.LevelUpCeremony` claim drives slow-mo during the hit beat; projectile physics respond to `Time.timeScale` normally. After the beat, a non-exclusive ramp-up (1 → `LevelCompleteRampUpScale`) speeds remaining flight |
| Trail spawning | Gated by `Phase != Playing` in `BalloonSpawner` — the tipping shot's spawn wave is cancelled |
...
| Score trails | Fly at normal speed during the flight — their arrivals must land to confirm the level-up. Frozen only once the ceremony is confirmed (`Phase → Pending`) |

### Pausing survivors through the ceremony

The level-up used to **clear** every still-airborne trail with `CompleteAll()` at the end of the hit beat.
...
1. **Freeze at confirmation, not at hit-beat start.** `LevelUpCinematic` subscribes to `ILevelProgress.Phase`
   ...
   *confirm* the level-up, and nothing else would force those arrivals — the popup would never fire (a
   soft-lock, worst for a small `DefaultScore` final pop where one trail alone can't reach the colour's
   threshold). Letting them fly until `Pending` means every confirming arrival has already landed, so the
   freeze can only ever catch pure visual survivors. The `ShapeFormationTicker` freezes each frozen
   formation and inflates its ribbons; `TrailFlight.Pause` does the same for a plain `DefaultScore` orb
   (`FlyingTrail.FreezeRibbon`) so its tail doesn't decay behind the popup.
...
3. **Abort/loss recover the controller too.** A loss committing mid-hit (`AbortSession`) still
...
| File | Role |
|---|---|
| `LevelUpCinematic.cs` | Level-up producer (plain C# `IStartable`): when `Phase == Completing` and a score point lands, begins the hit beat immediately (no trail wait). Camera follows the **projectile** (via `ProjectilePositionProvider`, box-in-box clamped within the play area) for the authored `LevelCompleteHit` curve duration — end is **duration-based**, not wall-hit triggered. At `EndPanIn` it starts a curve-driven camera restore (`Rig.RestoreCurveDriven` using the `LevelCompleteRestore` segment's curve, still following the projectile). On dismiss it only cleans up its own session — `LevelController` owns the nav return to `Game`. **The loss wins**: aborts mid-hit if `ILossForecast.LossImminent` becomes true, publishing `LevelUpAbortedMessage` so the pending ceremony can recover cleanly |
...
| `CinematicCameraRig.cs` | The **one shared** cinematic camera driver (DI singleton): `PreparePanIn(segment)` / `Frame(focus, segment, dt)` / `FollowClamped(target, outerBounds)` (box-in-box clamp: keeps the inner frustum inside the outer play-area bounds) / `PrepareRestore` (a runner segment step) / `RestoreCurveDriven(target, outerBounds, curve, onComplete)` (curve-sampled position/zoom interpolation back to base while following — used by `LevelUpCinematic.EndPanIn` for the post-hit un-zoom) / `RestoreTweened(duration)` (standalone tweened return that re-enables the ortho controller on completion) / `Restore` (instant). Not used by `LevelAscendCinematic`, which moves the scenario root instead (see above) |
