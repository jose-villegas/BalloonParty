...
| **Dismiss → restore** | The Restart button hides the screen and publishes `GameOverDismissedMessage`. The producer runs `TryBeginRestore` (`GameOverLossRestore`) — camera pulls back to base, timeScale eased to 1 |
| **Restart** | `OnEnded` (restore complete) resumes the pause and calls `RunController.RestartRun()`. **The restart is deferred to here** — not the button press |
...
| `GameOverLossCinematic.cs` | Loss-ceremony producer (plain C# `IStartable`), split around the screen like level-up: on `GameOverMessage`, arms `GameOverPresentationGate`, waits out any in-flight cinematic (the heart-drain), then `TryBegin` the `GameOverLoss` pan-in + `Pause(Cinematic)`. After the pan-in's curve duration it `EndPanIn`s (camera stays pushed in) and opens the gate → screen shows. On `GameOverDismissedMessage` it `TryBeginRestore`s (`GameOverLossRestore`, camera pulls back); `OnEnded` resumes the pause and calls `RunController.RestartRun()` — **the restart is deferred to the restore's end**. Opens the gate / restarts bare on any path where the beat can't run, so the screen never soft-locks |
| `Game/Run/GameOverPresentationGate.cs` | Plain DI singleton driving the loss reveal: starts closed, armed by the producer on game-over, opened when the pan-in hold ends. `GameOverScreen` awaits it before showing. Producer-driven rather than state-observing because the loss beat starts *after* its trigger message (it waits out the heart-drain). Not an `IReadyGate` — the producer drives it via `Arm`/`Open` and both sides inject the concrete type |
