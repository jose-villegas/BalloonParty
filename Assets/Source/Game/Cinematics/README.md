...
screen is up, and the camera only pulls back once the player dismisses — the run restarts when that
restore ends. It listens for `GameOverMessage` (published by `RunController.EndRun`), then:
...
| **Dismiss → restore + pop** | The Restart button hides the screen and publishes `GameOverDismissedMessage`. The producer snapshots the board, then runs the `GameOverLossRestore` camera pull-back **and** a `BoardPopWave` (the same slow-mo pop the Ascent uses) concurrently. The pull-back duration is overridden to the pop-wave's `EstimateSeconds()` (via `CameraRigCinematicConfig.RestoreDurationOverride`), so the camera settles as the board finishes popping — falling back to the authored `GameOverLossRestore` curve length only when the board is empty |
| **Restart** | Once *both* the pop wave and the camera restore finish (`UniTask.WhenAll`), the producer resumes the pause and calls `RunController.RestartRun()`. **The restart is deferred to here** — not the button press |
...
| `GameOverLossCinematic.cs` | Loss-ceremony producer (plain C# `IStartable`), split around the screen like level-up: on `GameOverMessage`, arms `GameOverPresentationGate`, waits out any in-flight cinematic (the heart-drain), then `TryBegin` the `GameOverLoss` pan-in + `Pause(Cinematic)`. After the pan-in's curve duration it `EndPanIn`s (camera stays pushed in) and opens the gate → screen shows. On `GameOverDismissedMessage` it runs the `GameOverLossRestore` camera pull-back **and** a `BoardPopWave` (shared with the Ascent) concurrently, then `RunController.RestartRun()` once both finish (`WhenAll`) — **the restart is deferred to the restore's end**. Opens the gate / restarts bare on any path where the beat can't run, so the screen never soft-locks |
| `IBoardEffect.cs` | Contract for a board-wide clear effect (plain C#): `Collect()` (snapshot while populated) / `EstimateSeconds()` (size a synced beat) / `PlayAsync(ct)`. Two implementations today — `BoardPopWave` and `BoardFloatAwayEffect` — with more expected. `LevelTransitionController` injects the interface (a swappable level-clear strategy, bound to the float effect); `GameOverLossCinematic` injects the concrete `BoardPopWave` |
