...
| `BoardPopWave.cs` | Shared slow-mo board pop (plain C#): snapshots balloons by anti-diagonal band and pops them from the far corners inward via the registry (scoreless — no `ActorHitMessage`), claiming `TimeScaleSource.LevelTransition` for the slow-mo. `Collect()` / `EstimateSeconds()` / `PlayAsync(ct)`. Used by both `LevelTransitionController` (the Ascent) and `GameOverLossCinematic` (the loss restore); tuning is the `LevelAscend` pop settings |
