...
| `ScoreTrailContext` | Readonly struct bundling everything a handler needs for one group: color/origin/hit direction/points/score range, the `ITrailEndpoint` target, `TrailSpawner`, `Flights` registry, a pooled `IScoreTrailReporter`, `IGameConfiguration`, and a group-scoped `CancellationToken` |
