...
- **Only a direct hit grows the streak.** Popping a balloon by hitting it with the shot itself extends the combo. Popping the same colour with a Bomb, Laser, or Lightning item does not push it higher — that pop still scores at whatever multiplier is already running (a bomb mid-combo isn't punished), and still resets the streak on a mismatched colour like any other pop; it just can't build the combo the player didn't earn by aiming. In code, that's `ScoreController.RecordStreakMultiplier` reading `DamageFlags.DirectHit` (true only for the shot's own hits) and passing it into `ColorStreakTracker.Record` as `canIncrement`
...
While it's night (`ITimeOfDayNight.IsNight`, see `Shared/SceneLight/README.md`), every pop is worth
double — a flat ×2 stacked on top of the streak multiplier in `PublishAttributionGroup`, before
`ClaimProgress` caps the total at the level threshold. A ×3 streak during the night window awards ×6.
Night is a global time-of-day state, not tied to any one color or balloon, so it applies uniformly
to whatever a pop attributes. `ColorProgressBar` shows a "2x" badge while it's active (see
`UI/Score/README.md`).
...
- **`ITimeOfDayNight`** (`Shared/SceneLight/`) — `ScoreController` reads `IsNight` to double every pop's points at night (see Night Multiplier above)
