...
| File | What it does |
|---|---|
| `RunController` | Plain C# singleton. `EndRun()` snapshots the final level/score, commits them to `IRunMeta`, publishes `GameOverMessage`, and transitions to `NavigationState.GameOver`. `RestartRun()` invokes every `IRunResettable` in ascending `ResetOrder`, then transitions back to `NavigationState.Game`. The single seam both the dev cheat and (later) the breach detector call to end a run |
