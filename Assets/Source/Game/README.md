...
`HitPipeline` (implementing `IHitDispatcher` from `Shared/Messages`) is the single entry point for actor hits. Producers — `ProjectileHitResolver`, the item handlers, cheats — never publish `ActorHitMessage` directly; they call `Dispatch`, which runs the order-dependent stages explicitly (`ScoreController` streak/score recording, then the owning balloon's reaction via `BalloonControllerRegistry`) and only then broadcasts the message for order-independent observers. This replaces two implicit contracts: the streak-shield rule's read-after-publish (now structural) and per-balloon broadcast filtering (now a flat-array index lookup via each balloon's `RegistryHandle`, not a dictionary).

`ScoreController` is score-keeping only — level progression lives in `LevelController` (`Game/Level/`). `ScoreController` handles hits as `HitPipeline`'s first stage (invoked directly, not bus-subscribed) and subscribes to `ScoreTrailArrivedMessage`. On a qualifying hit (`Pop` or `PassThrough`), it casts the actor to `IHasScoreColor` and calls `ResolveScoreAttribution(in msg.Context, incompleteColors, attributions)` — `incompleteColors` lists the palette colors still short of the level's threshold, so scatter implementers confine their random split to those bars; the actor appends one `ScoreAttribution(colorId, points, breaksStreak, isPrimary)` per color bar it contributes to. `BalloonModel` appends a single entry for its own color when `HitsRemaining` reaches zero; `ToughBalloonModel` scatters score across the incomplete colors on pop; `UnbreakableBalloonModel` attributes score to the source (projectile) color on pop; `BubbleClusterModel` scatters one entry per remaining bubble across the incomplete colors with `BreaksStreak = true` (no streak bonus, streak resets). For each resolved attribution `ScoreController` publishes one `ScorePointsGroupMessage` carrying the group's total granted `Points` (post-multiplier, post-cap) and `LastScore` (the cumulative per-color number of the group's last point); `ScoreTrailService` fans those points out into individual trails. `ScoreController`'s own score total only ticks up on `ScoreTrailArrivedMessage`, and it snaps to the run's fully-granted total the moment `ScoreLevelUpMessage` arrives, so the level-up popup never shows a stale low number while the last trails are still in flight. Score is run-scoped: no persistence beyond the best-run meta in `Run/`.

Level-up itself is a phased ceremony owned by `LevelController`: once every allowed color's *confirmed* progress (from `ScoreTrailArrivedMessage`) meets the level threshold — and the run is neither over nor doomed (`Navigation == Game`, `ILossForecast.LossImminent` false) — it publishes `ScoreLevelUpMessage` and moves `Navigation` to `LevelUp`. The level number and per-color progress don't advance yet at that point; they're deferred through a `LevelUpPhase` state machine (`Playing → Pending → Transitioning → Playing`) until the player dismisses the popup, and scoring only reopens once `LevelTransitionCompletedMessage` reports the level-up Ascent has settled.
...
- **GameLifetimeScope config** — `ProjectileFlightConfig`, `SlotGridConfig`, `PredictionTraceConfig`, `RunConfig`, `ProjectileVisualConfig`, `BalloonsConfiguration`, `GridActorConfiguration`, `GamePalette`, `GameDisplayConfiguration`, `ItemConfiguration`, `OverflowSettings`, `CinematicsSettings`, `PuffCloudSettings`, `BushSettings`, `DisturbanceFieldSettings`, `SceneLightFieldSettings`, `SpeckFieldSettings`, `LevelPacingConfiguration`, `BuffConfiguration`, `ScoreTrailBehaviourConfiguration` all registered here as singletons; `FlyingTrail` prefab and the thrower's `ProjectileView` prefab (wrapped in `ThrowerSettings`) registered as instances
