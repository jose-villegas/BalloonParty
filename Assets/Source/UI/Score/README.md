...
3. **`ScoreTrailService`** → spawns one pooled trail orb per point in the group, from the balloon's world position toward the bar
4. **Trail arrives** → `ScoreTrailService` publishes `ScoreTrailArrivedMessage` (`Points = 1` per trail this step)
...
- **ScoreController** — source of `ScorePointsGroupMessage`, `ScoreLevelUpMessage`, `TotalScore`, `Level`; score mutation deferred to `ScoreTrailArrivedMessage`
