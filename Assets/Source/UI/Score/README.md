...
Bar reset is a two-phase process synchronised with the fill trail ceremony:
...
2. **`LevelUpFillTrailsMessage`** → `DrainSliderAsync` gradually drains the slider to zero in sync with fill trail waves spawned by `LevelUpPopUp`.
...
- **LevelUpPopUp** — publishes `LevelUpFillTrailsMessage` (drain trigger) and `LevelUpDismissedMessage` (final reset)
