...
1. **Hit beat ends** — `LevelUpCinematic` ends the level-complete hit phase after the scoring projectile lands its first wall hit, setting `CinematicState` to `None`.
...
8. **Ascent + navigate** — the phase flip to `Transitioning` triggers `LevelTransitionController` (the Ascent), which un-zooms the camera (`CinematicCameraRig.RestoreTweened`, synced to its pop wave) and slides the new level in. `LevelUpCinematic.OnDismissed` resumes (`PauseSource.Cinematic`) and calls `Navigation.TransitionTo(Game)`. There is **no** `LevelCompleteRestore` cinematic — it's kept in the enum only for serialized-index stability.
...
- **`LevelUpCinematic`** — opens the gate by ending the level-complete hit beat; on `LevelUpDismissedMessage` resumes and navigates to `Game` (no restore cinematic — the camera un-zoom is the Ascent's)
