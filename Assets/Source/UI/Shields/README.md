...
| File | What it does |
|---|---|
| `ShieldUILifetimeScope` | VContainer child scope on the shield HUD root; registers `ShieldCounterLabel[]`, `ShieldCounterAnimation`, trail prefab, target provider, and `ShieldTrailController` entry point |
...
`ShieldTrailController` is a plain C# `IStartable` + `IDisposable` registered as an entry point in `ShieldUILifetimeScope`. It listens for `ShieldGainedMessage` (published by `ShieldItemHandler` when a shield item is activated). It resolves the balloon's world position from the slot index via `SlotGrid.IndexToWorldPosition`, then uses a composed `TrailSpawner` to spawn a `FlyingTrail` orb that tweens from that position to the target transform (registered in the scope as a `Func<Vector3>`). The trail duration is read from `IGameConfiguration.ShieldTrailDuration`. The spawner handles pool return on completion.
