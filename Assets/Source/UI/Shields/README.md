...
| `ShieldTrailController` | Subscribes to `ShieldGainedMessage`; spawns a pooled `ScorePointTrail` orb that flies from the balloon's slot position to the shield HUD |
| `ShieldTrailPoolChannel` | `PoolChannel<ScorePointTrail>` — pool keyed by `ShieldTrail` |
...
`ShieldTrailController` listens for `ShieldGainedMessage` (published by `ShieldItemHandler` when a shield item is activated). It resolves the balloon's world position from the slot index via `SlotGrid.IndexToWorldPosition`, then spawns a pooled `ScorePointTrail` that tweens from that position to the shield HUD's transform. The trail duration is read from `IGameConfiguration.ShieldTrailDuration`. On completion the trail is returned to the pool.
...
- **PoolManager** — `ShieldTrail` pool for `ScorePointTrail`; consumer handles return
