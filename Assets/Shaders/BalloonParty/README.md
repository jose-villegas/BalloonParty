...
| `BalloonParty/Grid/BushLeaf` | `Grid/BushLeaf.shader` | `BushView` — instanced unlit alpha-blend leaf shader for `DrawMeshInstanced`. Per-instance `_LeafTint` (color) and `_UVRect` (atlas sub-rect) via instancing buffer + `MaterialPropertyBlock`. Samples `_MainTex` (leaf atlas). 9-tap soft drop shadow via `_ShadowOffset`, `_ShadowColor`, `_ShadowSoftness` (material-level uniforms). Shadow composited behind the leaf via Porter-Duff "over". Material created at runtime by `BushView` from `IBushSettings.LeafShader`. GPU instancing **enabled** |
