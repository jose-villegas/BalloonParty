...
| `BalloonParty/Sprite/ShinyDefault` | `Sprite/SpriteShine.shader` | Specular shine overlay — diagonal shine band with opt-in "Shine Follows Scene Light" toggle (default = classic 45° diagonal), controlled by `_ShineLocation` and `_ShineWidth`. A second opt-in, **"Shine Uses Unscaled Time"** (`_ShineUnscaledTime`, default off), swaps `_Time.y` for the global `_BP_UnscaledTime` so the sweep keeps moving while `Time.timeScale` is 0 — the level-up popup and pause both freeze the scaled clock |
...
| `BalloonParty/Paint/PaintBlob` | `Paint/PaintBlob.shader` | `PaintBlob`, `PaintFlyingBlob` — procedural wobbling paint blob with dual-frequency sine wobble, rim darkening, specular highlight, and optional shadow. Specular hotspot and drop shadow are both world-anchored and derived from `_SceneLightDir` (shadow extends opposite direction). Per-material magnitude knobs: `_SpecularDistance`, `_ShadowDistance`, `_ShadowScale`. `_SpriteScale` controls content inset within the quad, preventing shadow clipping at edges. Optional `_CLOUD_FADE_ON` (the `SpriteLightDriven` ground-shadow pattern): the shadow fades toward `_CloudShadowFloor` where the shared `BackgroundField`/`SmokeField` reads empty behind it and shows fully where there's cloud/smoke to receive it — sampled per-fragment at the fragment's own (interpolated) world position, unlike the light direction/tint above: the shadow's shape comes from testing a shifted SDF query against the same quad, not an actually-displaced quad, so each pixel's true world position is unchanged and the fade can vary across the shadow's own footprint. Per-instance `_TimeOffset`, `_ShadowScale`, and `_SpriteScale` via `MaterialPropertyBlock` |
...
| `BalloonParty/Display/SkyScatter` | `Display/SkyScatter.shader` | The sky-scatter backdrop for `SkyScatterService` — renders on the stationary quad the service builds, sorted behind every other backdrop layer (`BackgroundCloud` sits at -10, this defaults to -20). Genuinely alpha-blended (`Blend One OneMinusSrcAlpha` — premultiplied, since the compositing loop already accumulates premultiplied colour; not opaque) — it enriches the camera's flat clear colour rather than replacing it, so wherever a ring fades to transparent the clear colour (already the current light colour, via `CameraBackgroundTint`) shows through. Naive 2D atmospheric scattering: a stack of circles (`_RingCount`, up to 10) all sharing one tangent point toward `SceneLight.cginc`'s `_SceneLightDir`, growing outward (`_BaseRadius`/`_RadiusGrowth`). Every ring is coloured off the SAME live `SceneLightTint()` — there is no separate authored "horizon" hue — with `_SelectedColor` applied only as a multiplier whose strength fades toward the outer rings (`_TintFalloff` shapes the falloff), and each ring's own opacity fading the same way from an authored `_BaseAlpha` at the innermost ring down to fully transparent at the outermost (`_AlphaFalloff` shapes that curve) — two independent knobs on the same near-to-far shape, since one is a colour multiplier and the other is real transparency. Painted back-to-front (standard "over" alpha compositing) so smaller rings layer on top; every ring shares the same anchor tangent point (its centre sits exactly one radius from the anchor), so growing radii nest automatically with no per-ring containment test. The shared tangent point orbits the circle that passes through every corner of the play-area rectangle (`BackgroundField.cginc`'s bounds, the same box `DisturbanceFieldCoordinates` builds — its circumscribed circle, radius = the half-diagonal), at the same angle as the scene light, scaled by `_AnchorRectScale` (1 = exactly that circle, &lt;1 = pulled inside the play field, &gt;1 = pushed out beyond it) — a fixed distance out that rotates with the light instead of sliding along the rectangle's edges. `_WaveAmplitude`/`_WaveSpeed`/`_WavePhaseStep` give each ring's radius a slow per-index sine breathing motion. Edges are perturbed by a single shared tap of `BackgroundField.cginc`'s `BackgroundFieldNoise` (`_CloudDistortion`, gated by `_BackgroundFieldActive`) so the rings read as organic rather than perfectly circular — reuses the existing cloud field instead of sampling new noise |
...
| Unscaled shader clock (`_BP_UnscaledTime`, pushed per frame by `ShaderTimeService` in `AppLifetimeScope`) | SpriteShine (opt-in) | `Include/ShineSweep.cginc` — `ShineTime(useUnscaled)` |
